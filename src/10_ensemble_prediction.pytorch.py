...
def main():
    ...
    if sparse.issparse(X_test_sparse):
        X_test_dense = X_test_sparse.toarray().astype("float32")
    else:
        X_test_dense = X_test_sparse
    ...
    print(f" -> Random Forest 基準準確率: {acc_rf:.2%}")
    ...
    plt.axvline(
        ...
        color="r",
        linestyle="--",
        label=f"Best Weight (RF={best_w_rf:.2f})",
    )
    ...
    plt.title(
        f"Optimized Ensemble Matrix (RF={best_w_rf:.2f}, Keras={1-best_w_rf:.2f})\nAccuracy: {best_weight_acc:.2%}"
    )
